...
def vanity_projects
  ...
  Movie.joins(:actors).where('castings.ord = 1').where('director_id = actors.id').select(:id, :title, 'actors.name')
end

def most_supportive
  # Find the two actors with the largest number of non-starring roles.
  # Show each actor's id, name, and number of supporting roles.
  Actor.joins(:castings).where.not(castings: {ord: 1}).group('actors.id').order('COUNT(castings.ord) DESC').limit(2).select(:id, :name, 'COUNT(castings.ord) AS roles')
end