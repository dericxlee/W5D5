def what_was_that_one_with(those_actors)
  ...
  Movie.joins(:actors).where(actors: {name: those_actors}).group('movies.id').having('COUNT(actors.id) = ?', those_actors.length).select(:title, :id)
end
...
def costars(name)
  ...
  films = Movie.joins(:actors).where(actors: {name: name}).select(:id)
  Actor.joins(:movies).where(movies: {id: films}).where.not('actors.name = ?', name).distinct.pluck(:name)
end

def actor_out_of_work
  # Find the number of actors in the database who have not appeared in a movie.
  Actor.left_outer_joins(:castings).where(castings: {actor_id: nil}).count
end
...
def starring(whazzername)
  ...
  Movie.joins(:actors).where('actors.name ILIKE ?', "%#{whazzername.split("").join("%")}%" ).select(:id,:title)
end
...
def longest_career
  ...
  Actor.select("actors.id, name, MAX(movies.yr) - MIN(movies.yr) AS career_length").joins(:movies).group(:id).order("career_length DESC, name").limit(3)
end